...
while true; do
    ...
    [1])
        while true; do
            ...
            case $yn in
            [Yy]*)
                cp -vur "${CODIUM_USER_CONFIG_PATH}/snippets/"* "${REPO_DIRECTORY}/Codium/User/snippets/"
    ...
    [2])
        while true; do
            ...
            case $yn in
            [Yy]*)
                cp -vur "${REPO_DIRECTORY}/Codium/User/"* "${CODIUM_USER_CONFIG_PATH}"
                break
                ;;
            [Nn]*) exit ;;
            esac
    ...
    [3])
        ...
        exec "${REPO_DIRECTORY}/scripts/extensions/extension.sh"
        break
        ;;
    [4])
